...
int minTimeToReach(vector<vector<int>>& moveTime) {
    ...
    vector<vector<bool>> isLastStepOne(m, vector<bool>(n, false));
    ...
    while (!toRelax.empty()) {
        ...
        if (j > 0 && !relaxed[i][j - 1] && max(costFromStart[i][j], moveTime[i][j - 1]) + 1 < costFromStart[i][j - 1]) {
            costFromStart[i][j - 1] = max(costFromStart[i][j], moveTime[i][j - 1]) + w;
            toRelax.push(make_pair(costFromStart[i][j - 1], i * n + (j - 1)));
            isLastStepOne[i][j - 1] = !isLastStepOne[i][j];
        }
